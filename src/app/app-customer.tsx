...
// ── أسباب التقييم السيء للسائق (تظهر فقط للزبائن) — الأكواد ثابتة، والتسمية تُترجم لاحقاً حسب اللغة ──
...
// ── صيغة التسعير النهائية: أساس ثابت + سعر تناقصي فعلياً للكيلومتر بفضل الأساس الثابت ──
...
// ════════════════════════════════════════════════════════════════════
// SearchRadar — pulsing circular "looking for driver" animation
// ════════════════════════════════════════════════════════════════════
...
type PlaceSuggestion = {
  ...
  mainText: string;       // النص الرئيسي (اسم المكان/الحي/الشارع)
  secondaryText: string;  // النص الثانوي (البلدية/الولاية)
};

// ⚠️ ضع هنا مفتاح Google API الخاص بك (فعّل عليه "Places API" من Google Cloud Console،
// ونفس المفتاح المستخدم لـ PROVIDER_GOOGLE بالخريطة يشتغل، بس لازم تفعّل عليه Places API وتفعّل الفوترة)
...
// يحوّل إحداثيات GPS إلى اسم مكان مقروء (الحي/الشارع) — يُستخدم لعرض اسم حقيقي
// بدل نص "موقعي الحالي" الثابت، بنفس أسلوب أي تطبيق نقل حقيقي
async function reverseGeocode(lat: number, lng: number): Promise<string | null> {
  try {
    ...
    // نستبعد أي نتيجة اسمها "شارع بدون اسم" (شائعة في المناطق الطرفية غير المُخرَّطة بتفصيل عند Google)
    ...
    // ترتيب الأولوية: اسم الحي/المنطقة أولاً (أوضح وأدق من اسم الشارع في المناطق الطرفية)،
    // ثم البلدية، ثم الشارع، وأخيراً أي نتيجة متاحة كحل أخير
...
// ── ترجمة كل نصوص شاشة الزبون (كانت عربي ثابت من الأول، دلوقتي مشتركة مع اللغة العامة) ──
...
export default function AppCustomer() {
  ...
  const [currentPlaceName, setCurrentPlaceName] = useState<string>(ct.myLocation); // نص افتراضي لحد ما يوصل الاسم الحقيقي
  ...
  const [suggestedPrice, setSuggestedPrice] = useState<number | null>(null); // السعر اللي اقترحه التطبيق تلقائياً
  const [customerPrice,  setCustomerPrice]  = useState<number | null>(null); // السعر النهائي بعد تعديل الزبون (إن وُجد)

  // ── نظام تقييم/تحذير السائقين (مرآة لنظام customerReports في app-driver.tsx) ──
  ...
  // ── Real-time: listen to offers made by drivers on this ride ──
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const offers: Driver[] = snap.docs.map((d) => {
        const o = d.data();
        return {
          id:     o.driverId ?? d.id, // driverId الحقيقي (uid) إن وُجد، وإلا معرّف العرض كحل احتياطي
  ...
  // ── Real-time: نظام تحذير السائقين — نفس نمط customerReports بالضبط ──
  useEffect(() => {
  ...
  // ── Real-time: listen to the ride document itself for status changes ──
  useEffect(() => {
    if (!rideId) return;
  ...
  // ── لحظة اكتمال الرحلة، افتح نافذة تقييم السائق تلقائياً ──
  useEffect(() => {
  ...
  // ── Real-time: live GPS location of the chosen driver ──
  useEffect(() => {
  ...
  const pickSuggestion = async (p: PlaceSuggestion) => {
    ...
    setSuggestions([]);
    setSearchAttempted(false); // ← يمنع بقاء رسالة "لا توجد نتائج" من بحث سابق ظاهرة بالغلط
    pendingQueryRef.current = '';
    Keyboard.dismiss();
    ...
    if (coords) {
      ...
      if (location) {
        ...
        setCustomerPrice(price); // نبدأ دائماً بالسعر المقترح، والزبون حر يعدّله بعدها
      }
    } else {
  ...
  const searchDrivers = async () => {
    ...
    // احتياطي: لو لسبب ما suggestedPrice/customerPrice لسه فاضيين (نادر)، احسبهم دلوقتي
    ...
    try {
      const rideRef = await addDoc(collection(db, 'rides'), {
        ...
        pickupAddress:   'موقع الزبون الحالي',
        destLat:         dest.latitude,
        destLng:         dest.longitude,
        destination,
        price:           finalPrice,     // السعر النهائي (بعد تعديل الزبون إن وُجد)
        suggestedPrice:  finalSuggested, // السعر الأصلي المقترح من التطبيق — للمقارنة عند السائق
        status:          'pending',
  ...
  // ── تحديث تقييم السائق — مطابقة تامة لـ updateCustomerStatus في app-driver.tsx ──
    ...
    <View style={s.container}>
      ...
      {stage === 'idle' && (
        ...
        >
          ...
          {/* ── تعديل السعر — الزبون حر يزوّد أو ينقّص السعر المقترح ── */}
      ...
      {stage === 'choosing' && (
        <View style={s.bottomSheetTall}>
          ...
          <ScrollView showsVerticalScrollIndicator={false}>
            {drivers.map(d => {
              // ⚠️ تحذير قبل اتخاذ القرار — لو هذا السائق تلقى تقييمات سيئة من زبائن سابقين
                ...
                >
                  ...
                  <View style={s.driverInfo}>
                    ...
                    <View style={[s.driverMeta, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                      <Text style={s.driverRating}>★ {d.rating}</Text>
      ...
      {/* ── TRACKING: تتبع حقيقي لموقع السائق ── */}
          ...
          <View style={[s.trackingCard, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
            ...
            <View style={s.trackingInfo}>
              ...
              <View style={[s.trackingRatingRow, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
                <Text style={s.trackingRating}>★ {chosenDriver.rating}</Text>
      ...
      {/* ── نافذة تقييم السائق — تظهر تلقائياً بعد وصول الرحلة ── */}
      <RatingModal
