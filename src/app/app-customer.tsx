...
export default function AppCustomer() {
  ...
  const [pickupText,   setPickupText]   = useState(ct.myLocation);
    ...
    (async () => {
      ...
      reverseGeocode(newLocation.latitude, newLocation.longitude).then((name) => {
        if (name) {
          ...
          if (!pickupManuallyEdited) setPickupText(name);
        }
      });
  ...
  const searchDrivers = async () => {
    ...
    const origin: LatLng = pickupCoords ?? location;
      ...
      {stage === 'choosing' && (
        <View style={s.bottomSheetTall}>
          <View style={[s.sheetRow, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
            <Text style={[s.sheetTitle, { textAlign: isRTL ? 'right' : 'left' }]}>
