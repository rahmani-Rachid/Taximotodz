...
const MANEUVER_TEXT: Record<Lang, Record<string, string>> = {
  ...
  fr: {
    ...
    'depart':            'Départ',
    default:             'Continuez sur la route',
  },
  en: {
    ...
    'depart':            'Depart',
    default:             'Continue on route',
  },
};

function maneuverToText(type: string, modifier: string | undefined, lang: Lang): string {
  const map = MANEUVER_TEXT[lang];
  if (type === 'arrive') return map['arrive'];
  if (type === 'depart') return map['depart'];
  if (type === 'roundabout' || type === 'rotary') return map['roundabout'];
  if (type === 'merge') return map['merge'];
  if (type === 'turn' || type === 'end of road' || type === 'fork') {
    const key = modifier ? `turn-${modifier}` : 'turn-straight';
    return map[key] ?? map['turn-straight'];
  }
  return map.default;
}
...
async function fetchRouteWithSteps(
  ...
  toLat: number,   toLng: number,
  lang: Lang,
): Promise<{ coords: LatLng[]; steps: NavStep[] }> {
  try {
    const url =
      `https://router.project-osrm.org/route/v1/driving/` +
      `${fromLng},${fromLat};${toLng},${toLat}` +
    ...
    const res  = await fetch(url);
    const json = await res.json();
    if (json.code !== 'Ok') return { coords: [], steps: [] };
    ...
    for (const leg of json.routes[0].legs ?? []) {
      for (const step of leg.steps ?? []) {
        const [lng, lat] = step.maneuver?.location ?? [null, null];
        if (lat == null || lng == null) continue;
        steps.push({
          location: { latitude: lat, longitude: lng },
          instruction: maneuverToText(step.maneuver?.type, step.maneuver?.modifier, lang),
          distanceM: step.distance ?? 0,
        });
      }
    }
    return { coords, steps };
  } catch { return { coords: [], steps: [] }; }
}
...
export default function AppCustomer() {
 ...
 useEffect(() => {
   ...
   const unsub = onSnapshot(doc(db, 'rides', rideId), (snap) => {
     ...
     if (data.status === 'accepted') {
       if (!acceptedFiredRef.current) {
         acceptedFiredRef.current = true;
         triggerArrivalAlert(); // نفس صوت الجرس + الاهتزاز، يُستخدم هنا لتنبيه قبول السائق أيضاً
       }
 ...
 useEffect(() => {
   ...
   const unsub = onSnapshot(doc(db, 'drivers', chosenDriver.id), (snap) => {
     ...
     const newPos = { latitude: d.lat, longitude: d.lng };
     setDriverPos(newPos);

     // تحريك الكاميرا لتتبع السائق: تكبير + إمالة + تدوير حسب اتجاه الحركة (كما في InDrive/Uber)
     if (mapRef.current) {
       const prev = prevDriverPosRef.current;
       const heading = prev ? computeBearing(prev, newPos) : 0;
     ...
     prevDriverPosRef.current = newPos;

     // تحديث خطوة التوجيه الحالية بناءً على أقرب خطوة قادمة لموقع السائق
     if (navSteps.length > 0) {
       let closestIdx = currentStepIdx;
       let closestDist = Infinity;
       for (let i = currentStepIdx; i < navSteps.length; i++) {
         const dist = haversineKm(newPos.latitude, newPos.longitude, navSteps[i].location.latitude, navSteps[i].location.longitude);
         if (dist < closestDist) { closestDist = dist; closestIdx = i; }
         if (dist < 0.03) break; // وصل لهذه الخطوة تقريباً — توقف عن البحث أبعد
       }
 ...
 const searchDrivers = async () => {
   ...
   const { coords, steps } = await fetchRouteWithSteps(origin.latitude, origin.longitude, dest.latitude, dest.longitude, lang);
 ...
 return (
   <View style={s.container}>
     ...
     {stage === 'tracking' && navSteps[currentStepIdx] && (
       <View style={s.navBanner}>
         <Text style={s.navBannerIcon}>🧭</Text>
         <View style={{ flex: 1 }}>
           <Text style={s.navBannerText} numberOfLines={1}>{navSteps[currentStepIdx].instruction}</Text>
           <Text style={s.navBannerDist}>{Math.round(navSteps[currentStepIdx].distanceM)} م</Text>
         </View>
       </View>
     )}
...
const s = StyleSheet.create({
 ...
 navBanner:          { position:'absolute', top:100, left:16, right:16, backgroundColor:'#1F2A40', borderRadius:16, padding:14, flexDirection:'row', alignItems:'center', gap:12, elevation:8, shadowColor:'#000', shadowOpacity:0.25, shadowOffset:{width:0,height:3}, shadowRadius:8 },
 navBannerIcon:      { fontSize:26 },
 navBannerText:      { fontSize:15, fontWeight:'800', color:'#fff' },
 navBannerDist:      { fontSize:12, color:'#E8B84B', marginTop:2, fontWeight:'700' },
